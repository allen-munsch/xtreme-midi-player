...
const INSTRUMENT_KEY_ARRAY = [
	...
	"0391_GeneralUserGS_sf2_file",
	"0391_SoundBlasterOld_sf2",
	"0392_FluidR3_GM_sf2_file",
	"0392_GeneralUserGS_sf2_file",
	...
	"0530_SBLive_sf2",
	"0530_Soul_Ahhs_sf2_file",
	"0530_SoundBlasterOld_sf2",
	"0531_FluidR3_GM_sf2_file",
	"0531_GeneralUserGS_sf2_file",
	...
	"0601_GeneralUserGS_sf2_file",
	"0602_GeneralUserGS_sf2_file",
	"0603_GeneralUserGS_sf2_file",
	...
	"0661_FluidR3_GM_sf2_file",
	"0661_GeneralUserGS_sf2_file",
	...
	"0761_GeneralUserGS_sf2_file",
	"0761_SoundBlasterOld_sf2",
	"0762_GeneralUserGS_sf2_file",
	...
	"0771_GeneralUserGS_sf2_file",
	"0772_GeneralUserGS_sf2_file",
	...
	"0791_GeneralUserGS_sf2_file",
	"0800_Aspirin_sf2_file",
	"0800_Chaos_sf2_file",
	"0800_FluidR3_GM_sf2_file",
	"0800_GeneralUserGS_sf2_file",
	"0800_JCLive_sf2_file",
	"0800_SBLive_sf2",
	"0800_SoundBlasterOld_sf2",
	"0801_FluidR3_GM_sf2_file",
	"0801_GeneralUserGS_sf2_file",
	...
	"0822_GeneralUserGS_sf2_file",
	"0823_GeneralUserGS_sf2_file",
	...
	"0871_GeneralUserGS_sf2_file",
	"0872_GeneralUserGS_sf2_file",
	"0873_GeneralUserGS_sf2_file",
	...
	"0882_FluidR3_GM_sf2_file",
	"0882_GeneralUserGS_sf2_file",
	"0883_GeneralUserGS_sf2_file",
	"0884_GeneralUserGS_sf2_file",
	"0885_GeneralUserGS_sf2_file",
	"0886_GeneralUserGS_sf2_file",
	"0887_GeneralUserGS_sf2_file",
	"0888_GeneralUserGS_sf2_file",
...
export const createWebAudioFontPlayer = () => {
	const envelopes = [];
	const loader = createWebAudioFontLoader();
	let onCacheFinish = null;
	let onCacheProgress = null;
	const afterTime = 0.05;
	const nearZero = 0.000001;

	const createChannel = (audioContext) => createWebAudioFontChannel(audioContext);
	const createReverberator = (audioContext) => createWebAudioFontReverberator(audioContext);

	const limitVolume = (volume) => (volume ? volume * 1.0 : 0.5);

	const queueChord = (audioContext, target, preset, when, pitches, duration, volume, slides) => {
	...
	const queueStrumUp = (audioContext, target, preset, when, pitches, duration, volume, slides) => {
		const sortedPitches = [...pitches].sort((a, b) => b - a);
		queueStrum(audioContext, target, preset, when, sortedPitches, duration, volume, slides);
	};

	const queueStrumDown = (audioContext, target, preset, when, pitches, duration, volume, slides) => {
		const sortedPitches = [...pitches].sort((a, b) => a - b);
		queueStrum(audioContext, target, preset, when, sortedPitches, duration, volume, slides);
	};

	const queueStrum = (audioContext, target, preset, when, pitches, duration, volume, slides) => {
		volume = limitVolume(volume);
	...
	const queueSnap = (audioContext, target, preset, when, pitches, duration, volume, slides) => {
		volume = limitVolume(volume) * 1.5;
		duration = 0.05;
		queueChord(audioContext, target, preset, when, pitches, duration, volume, slides);
	};

	const queueWaveTable = (audioContext, target, preset, when, pitch, duration, volume, slides) => {
		...
		let playbackRate = Math.pow(2, (100.0 * pitch - baseDetune) / 1200.0);
		const sampleRatio = zone.sampleRate / audioContext.sampleRate;
		let startWhen = Math.max(when, audioContext.currentTime);
		const waveDuration = duration + afterTime;
		...
		if (slides && slides.length > 0) {
			...
			slides.forEach((slide) => {
				const newPlaybackRate = Math.pow(2, (100.0 * slide.pitch - baseDetune) / 1200.0);
	...
	const noZeroVolume = (n) => Math.max(n, nearZero);

	const setupEnvelope = (audioContext, envelope, zone, volume, when, sampleDuration, noteDuration) => {
		envelope.gain.setValueAtTime(noZeroVolume(0), audioContext.currentTime);
		let lastTime = 0;
		let lastVolume = 0;
		let duration = Math.min(noteDuration, sampleDuration - afterTime);

		// Ensure ahdsr is always an array with at least one element
		const ahdsr =
			zone.ahdsr && zone.ahdsr.length > 0
				? zone.ahdsr
				: [
						{ duration: 0, volume: 1 },
	...
	const numValue = (aValue, defValue) => (typeof aValue === "number" ? aValue : defValue);

	const findEnvelope = (audioContext, target, when, duration) => {
		let envelope = envelopes.find(
			(e) => e.target === target && audioContext.currentTime > e.when + e.duration + 0.001,
		);
		if (envelope) {
			try {
	...
	const adjustPreset = (audioContext, preset) => {
		preset.zones.forEach((zone) => adjustZone(audioContext, zone));
	};
	const adjustZone = (audioContext, zone) => {
		if (!zone.buffer) {
			zone.delay = 0;
			if (zone.sample) {
				...
				for (let i = 0; i < decoded.length / 2; i++) {
					const b1 = decoded.charCodeAt(i * 2);
					const b2 = decoded.charCodeAt(i * 2 + 1);
					const n = b2 * 256 + b1;
					float32Array[i] = (n >= 65536 / 2 ? n - 65536 : n) / 65536.0;
				}
			} else if (zone.file) {
			...
			zone.sustain = numValue(zone.sustain, 0);

			// Initialize ahdsr if not present
			if (!zone.ahdsr || zone.ahdsr.length === 0) {
				zone.ahdsr = [
					{ duration: 0, volume: 1 },
	...
	const findZone = (audioContext, preset, pitch) => {
		const zone = preset.zones.reverse().find((z) => z.keyRangeLow <= pitch && z.keyRangeHigh + 1 >= pitch);
		try {
	...
	const cancelQueue = (audioContext) => {
		envelopes.forEach((e) => {
			e.gain.cancelScheduledValues(0);
			e.gain.setValueAtTime(nearZero, audioContext.currentTime);
			e.when = -1;
			try {
...
export const createWebAudioFontLoader = (player) => {
	const cached = [];

	const startLoad = (audioContext, filePath, variableName) => {
		if (window[variableName]) {
			return;
		}
		if (cached.some((item) => item.variableName === variableName)) {
			return;
		}
		cached.push({ filePath, variableName });
	...
	const decodeAfterLoading = (audioContext, variableName) => {
		waitOrFinish(variableName, () => {
			player.adjustPreset(audioContext, window[variableName]);
		});
	};

	const waitOrFinish = (variableName, onFinish) => {
		if (window[variableName]) {
			onFinish();
		} else {
			setTimeout(() => waitOrFinish(variableName, onFinish), 111);
		}
	};

	const loaded = (variableName) => {
		if (!window[variableName]) {
			return false;
		}
		const preset = window[variableName];
		return preset.zones.every((zone) => zone.buffer);
	};

	const progress = () => {
		if (cached.length > 0) {
			const loadedCount = cached.filter((item) => loaded(item.variableName)).length;
			return loadedCount / cached.length;
		}
		return 1;
	};

	const waitLoad = (onFinish) => {
		if (progress() >= 1) {
			onFinish();
		} else {
			setTimeout(() => waitLoad(onFinish), 333);
		}
	};
	...
	const instrumentInfo = (n) => {
		const key = INSTRUMENT_KEY_ARRAY[n];
		const p = parseInt(key.substr(0, 3), 10);
		return {
			variable: `_tone_${key}`,
			url: `https://surikov.github.io/webaudiofontdata/sound/${key}.js`,
			title: instrumentTitles()[p],
		};
	};

	const findInstrument = (program) => {
		return INSTRUMENT_KEY_ARRAY.findIndex((key) => parseInt(key.substring(0, 3), 10) === program) || 0;
	};
	...
	const drumInfo = (n) => {
		const key = DRUM_KEY_ARRAY[n];
		const p = parseInt(key.substr(0, 2), 10);
		return {
			variable: `_drum_${key}`,
	...
	const findDrum = (nn) => {
		return DRUM_KEY_ARRAY.findIndex((key) => parseInt(key.substring(0, 2), 10) === nn) || 0;
	};
...
export const createWebAudioFontChannel = (audioContext) => {
	console.log("WebAudioFont Channel v1.04");

	const createBandEqualizer = (from, frequency) => {
	...
	const input = audioContext.createDynamicsCompressor();
	input.threshold.setValueAtTime(-3, 0);
	input.knee.setValueAtTime(30, 0);
	input.ratio.setValueAtTime(12, 0);
	input.attack.setValueAtTime(0.05, 0);
	input.release.setValueAtTime(0.08, 0);

	const band32 = createBandEqualizer(input, 32);
	const band64 = createBandEqualizer(band32, 64);
	const band128 = createBandEqualizer(band64, 128);
	const band256 = createBandEqualizer(band128, 256);
	const band512 = createBandEqualizer(band256, 512);
	const band1k = createBandEqualizer(band512, 1024);
	const band2k = createBandEqualizer(band1k, 2048);
	const band4k = createBandEqualizer(band2k, 4096);
	const band8k = createBandEqualizer(band4k, 8192);
	const band16k = createBandEqualizer(band8k, 16384);

	const output = audioContext.createGain();
	band16k.connect(output);
...
export const createWebAudioFontReverberator = (audioContext) => {
	console.log("WebAudioFont Reverberator v1.09");

	const input = audioContext.createBiquadFilter();
	input.type = "lowpass";
	input.frequency.setTargetAtTime(18000, 0, 0.0001);

	const output = audioContext.createGain();

	const dry = audioContext.createGain();
	dry.gain.setTargetAtTime(0.9, 0, 0.0001);
	dry.connect(output);

	const wet = audioContext.createGain();
	wet.gain.setTargetAtTime(0.5, 0, 0.0001);
	...
	const irr = IRR; // Base64 encoded impulse response
	const datalen = irr.length / 2;
	const irrArrayBuffer = new ArrayBuffer(datalen);
	const view = new Uint8Array(irrArrayBuffer);
	...
	audioContext.decodeAudioData(irrArrayBuffer, (audioBuffer) => {
	...
	return {
		...
		dry,
		wet,
		getConvolver: () => convolver,
	};
};

if (typeof window !== "undefined") {
	window.createWebAudioFontPlayer = createWebAudioFontPlayer;
	window.createWebAudioFontLoader = createWebAudioFontLoader;
	window.createWebAudioFontChannel = createWebAudioFontChannel;
	window.createWebAudioFontReverberator = createWebAudioFontReverberator;
}
