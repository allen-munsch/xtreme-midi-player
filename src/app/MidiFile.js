var UTF8 = {
	...
	setBytesFromCharCode: function (charCode, bytes, byteOffset, neededBytes) {
		...
		if (1 == neededBytes) {
			...
			bytes[byteOffset++] =
				(parseInt("1111".slice(0, neededBytes), 2) << (8 - neededBytes)) + (charCode >>> (--neededBytes * 6));
...
MIDIFile.prototype.parseSong = function () {
	...
	events.forEach((event) => {
		song.duration = Math.max(song.duration, event.playTime / 1000);

		switch (event.subtype) {
			case MIDIEvents.EVENT_MIDI_NOTE_ON:
				this.handleNoteOn(event, song);
				break;

			case MIDIEvents.EVENT_MIDI_NOTE_OFF:
				this.handleNoteOff(event, song);
				break;

			case MIDIEvents.EVENT_MIDI_PROGRAM_CHANGE:
				this.handleProgramChange(event, song);
				break;

			case MIDIEvents.EVENT_MIDI_CONTROLLER:
				this.handleController(event, song);
				break;

			case MIDIEvents.EVENT_MIDI_PITCH_BEND:
				this.addSlide(event, song);
				break;

			case MIDIEvents.EVENT_MIDI_CHANNEL_AFTERTOUCH:
				this.handleChannelAftertouch(event, song);
				break;

			default:
				console.log("Unknown event", event.subtype, "on channel", event.channel, event);
		}
	});
...
MIDIFile.prototype.handleNoteOn = function (event, song) {
	if (event.channel === 9) {
		if (event.param1 >= 35 && event.param1 <= 81) {
			this.startDrum(event, song);
		} else {
			console.log("Invalid drum note", event);
		}
	} else {
		if (event.param1 >= 0 && event.param1 <= 127) {
			this.startNote(event, song);
		} else {
			console.log("Invalid note", event);
		}
	}
};

MIDIFile.prototype.handleNoteOff = function (event, song) {
	if (event.channel !== 9) {
		this.closeNote(event, song);
	}
};

MIDIFile.prototype.handleProgramChange = function (event, song) {
	if (event.channel !== 9) {
		const track = this.takeTrack(event.channel, song);
		track.program = event.param1;
	} else {
		console.log("Skipping program change for drums");
	}
};

MIDIFile.prototype.handleController = function (event, song) {
	if (event.param1 === 7 && event.channel !== 9) {
		const track = this.takeTrack(event.channel, song);
		track.volume = event.param2 / 127 || 0.000001;
	}
};

MIDIFile.prototype.handleChannelAftertouch = function (event, song) {
	console.log("Channel Aftertouch", event.channel, event.param1);
	// Implement channel aftertouch handling if needed
};
